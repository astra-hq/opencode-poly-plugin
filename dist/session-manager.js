...
class SessionManager {
    ...
    constructor(opencodeCreateSession) {
     ...
     */
    async onMeetingScheduled(payload) {
        const session = {
     ...
     */
    async onMeetingStarted(payload) {
        ...
        const sessionId = await this.opencodeCreateSession(`poly-meeting-${payload.meeting_id}`, context);
    ...
    onMeetingPaused(payload) {
        const session = this.sessions.get(payload.meeting_id);
        if (session) {
            session.status = "paused";
        }
    }
    onMeetingResumed(payload) {
        const session = this.sessions.get(payload.meeting_id);
        if (session) {
            session.status = "active";
        }
